...
def compute_accuracy(p: EvalPrediction):
    ...
    for pred,true_id in zip(preds,p.label_ids):
        ...
        counter+=1
    with open(f"logs/pred_out/lora/user_{group_key}_pred.json","w") as fp:
...
def train_module(train, test, uniq_lbl_count, adapter_name,freeze):
    ...
    # model.add_tagging_head(adapter_name, num_labels=uniq_lbl_count)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o","--out_file",help="output_file_name", default="logs/lora_eval.json")
    parser.add_argument("-if","--in_file",help="input_file_name", default="datasets/go_emotion_full_input.json")
